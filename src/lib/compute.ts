...
export function sessionSetCount(s: WorkoutSession): number {
	return s.exercises.reduce((a, e) => a + e.sets.filter((x) => x.completed).length, 0);
}
...
/** treadmill distance in km = speed(km/h) × time(s) / 3600. Derived, never stored. */
export function cardioDistanceKm(s: LoggedSet): number | undefined {
	if (s.speed == null || s.timeSec == null) return undefined;
	return (s.speed * s.timeSec) / 3600;
}
...
export function templateDerived(t: Template, byId: Map<string, Exercise>): TemplateDerived {
	...
	for (const te of t.exercises) {
		...
		for (const ps of te.plannedSets) {
			setCount++;
			seconds += WORK_PER_SET_SEC + (ps.targetRestSec ?? ex.defaultRestSec ?? 60);
		}
	}
	return {
		muscles: [...muscles],
