...
function weeksInWindow(
  ...
  const first = sessions.reduce(
    (min, s) => Math.min(min, Date.parse(s.startedAt)),
    now.getTime()
  );
  return Math.max(
    1,
    Math.round(
      (startOfWeek(now).getTime() - startOfWeek(new Date(first)).getTime()) /
        WEEK
    ) + 1
  );
}
...
export function kpis(sessions: WorkoutSession[], now = new Date()): Kpis {
  ...
  const vol = (a: WorkoutSession[]) =>
    a.reduce((x, s) => x + sessionVolume(s), 0);
  const time = (a: WorkoutSession[]) =>
    a.reduce((x, s) => x + (sessionDurationSec(s) ?? 0), 0);
...
export function weeklyVolume(
  ...
  return {
    weeks: buckets.map((v, i) => ({
      l: md(addDays(base, i * 7)),
...
export function heatmap(
  ...
  return {
    ...
    perWeek: Math.round((list.length / n) * 10) / 10,
  };
}
...
export function progression(
  ...
  for (const s of list) {
    ...
    if (isTime) {
      ...
      if (mReps > bestReps)
        (bestReps = mReps), (bestRepsW = mRepsW), (bestRepsDate = s.startedAt);
  ...
  const fmt = (n: number) =>
    hasWeight ? `${kg(n)}` : isTime ? mmss(n) : `${n}`;
  ...
  for (const id of ids) {
    ...
    for (const s of asc) {
      ...
      const m = isTime
        ...
        : isCardio
        ? Math.max(...sets.map((x) => (cardioDistanceKm(x) ?? 0) * 1000)) // furthest, in metres
...
export function pushPull(
  ...
  return {
    ...
    push: push.slice(start),
    pull: pull.slice(start),
  };
}
